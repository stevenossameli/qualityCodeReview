...
public class FilterArticleByField implements IFilterByField<ArticleDTO, String> {
    ...
    public List<ArticleDTO> filterHandler(List<ArticleDTO> toFilter, String filterName, String value) throws InvalidRequestParam, NotFoundException {
        ...
        if(filterName.equalsIgnoreCase("category"))   return filter.filterBy(toFilter, ArticleDTO -> ArticleDTO.getCategory().equalsIgnoreCase(value));
        if(filterName.equalsIgnoreCase("productName") || filterName.equalsIgnoreCase("name")) return filter.filterBy(toFilter, ArticleDTO -> ArticleDTO.getName().equalsIgnoreCase(value));
        if(filterName.equalsIgnoreCase("sendFree"))  return filter.filterBy(toFilter, ArticleDTO::getSendFree);
        if(filterName.equalsIgnoreCase("trademark")) return filter.filterBy(toFilter, ArticleDTO -> ArticleDTO.getTradeMark().equalsIgnoreCase(value));
        if(filterName.equalsIgnoreCase("prestige")) return filter.filterBy(toFilter, ArticleDTO -> ArticleDTO.getPrestige() == Integer.parseInt(value));
        if(filterName.equalsIgnoreCase("order")) {sorterHandler.sort(toFilter, value); return toFilter;}
